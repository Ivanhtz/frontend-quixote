...
export class SalesFormComponent implements OnInit {
  ...
  async getDataForm() {
    console.log('antes status',this.status)
    console.log('antes textMail',this.textMail)
    console.log('antes director',this.mailDirector)
    console.log('antes register',this.register.value.status)
    // Controlamos si el pedido llega sin destinatario de mail y asignamos uno vacio
    if (this.status !== 'pendiente' && this.textMail !== 'pendiente') {
      this.mailDirector = 'nomail@none.non';
    }
    console.log('despues textMail',this.textMail)
    console.log('despues director',this.mailDirector)
    console.log('despues register',this.register.value.status)
    // Controlamos si el selec llega vacio y asignamos un mail vacio
    if (this.mailDirector.length <= 0) {
      this.mailDirector = 'nomail@none.non';
    }
    ...
    if (this.register.value.id_order) {
      try {
        ...
        if (response.affectedRows === 1) {
          ...
          alert('No añadiste un destinatario');
        }
